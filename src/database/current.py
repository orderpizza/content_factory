...
def initialize_database(path):
    """Create empty current schema or validate it; never convert another schema."""
    path = Path(path).resolve()
    ...
    try:
        ...
        try:
            ...
            connection.execute("INSERT INTO schema_migrations(version,name,checksum,applied_at) VALUES (?,?,?,?)",
                               (SCHEMA_VERSION, SCHEMA_NAME, contract_checksum(), datetime.now(timezone.utc).isoformat()))
            validate_database(connection)
            connection.commit()
        except Exception:
            connection.rollback()
